...
class PerturbationTransformerWithEmbeddingModel(PerturbationModel):
    """
    Transformer-based perturbation prediction model.
    ...
    def __init__(
        ...
        hvg_mask: torch.Tensor,
        d_h: int = 672,  # Hidden dimension
        d_emb: int = 2058, # State SE embedding dimension
        n_genes: int = 18080,  # Number of genes (G)
        pert_emb_dim: int = 5120,  # Perturbation embedding dimension
        n_cell_types: int = None,  # Number of cell types (will be set from data)
        n_batches: int = None,  # Number of batches (will be set from data)
        ...
        **kwargs
        ...
        # Basal encoder for control cells
        self.basal_encoder = MLP(d_emb, d_h, d_h, dropout=dropout, n_layers=4)
        ...
        self.transformer, self.model_dim = get_transformer_backbone("llama", transformer_kwargs, input_dim=d_emb)
        
        # Projection layers (as specified)
        self.project_out = MLP(d_h, d_h, d_emb, dropout=dropout, n_layers=2)
        self.emb_to_gene = MLP(d_emb, d_emb, n_genes, dropout=dropout, n_layers=2)

        # build mlp for final down-then-up projection
        self.final_down_then_up = MLP(n_genes, d_x, n_genes, dropout=dropout, n_layers=2)
    ...
    def forward(self, ctrl_cell_emb, pert_emb, covariates):
        ...
        Returns:
            Predicted perturbation effects [B, S, G]
        """
        B, S, D = ctrl_cell_emb.shape
        
        # Initialize embeddings if needed
        self._initialize_embeddings_if_needed(covariates)
        
        # Encode control cells: [B, S, D] -> [B, S, d_h]
        H_ctrl = self.basal_encoder(ctrl_cell_emb)  # [B, S, d_h]
        ...
        # Project to gene space
        emb_proj = self.project_out(O)  # [B, S, D]
        gene_proj = self.emb_to_gene(emb_proj)  # [B, S, G]
        X_pert_hat = self.final_down_then_up(gene_proj)  # [B, S, G]
        # clip X_pert_hat to be non-negative
        X_pert_hat = torch.clamp(X_pert_hat, min=0)
        
        return X_pert_hat, emb_proj
    ...
    def unpack_batch(self, batch: dict[str, Any]):
        ...
        pert_cell_emb = batch["pert_cell_emb"].to_dense()  # [B, S, D]
        ctrl_cell_emb = batch["ctrl_cell_emb"].to_dense()  # [B, S, D]
        pert_cell_g = batch["pert_cell_g"].to_dense()  # [B, S, G]
        ctrl_cell_g = batch["ctrl_cell_g"].to_dense()  # [B, S, G]
        ...
        return pert_cell_emb, ctrl_cell_emb, pert_cell_g, ctrl_cell_g, pert_emb, covariates 

    def training_step(self, batch, batch_idx):
        """Training step with MMD loss."""
        # Unpack batch
        pert_cell_emb, ctrl_cell_emb, pert_cell_g, ctrl_cell_g, pert_emb, covariates = self.unpack_batch(batch)
        
        # Forward pass
        X_pert_hat, X_pert_emb_hat = self.forward(ctrl_cell_emb, pert_emb, covariates)

        # Compute MMD loss between predicted and actual perturbed cells
        # Reshape for loss computation: [B, S, G] -> [B*S, G]
        B, S, G = X_pert_hat.shape
        _, _, D = X_pert_emb_hat.shape

        X_pert_hat = X_pert_hat.reshape(-1, S, G).contiguous()
        pert_cell_g = pert_cell_g.reshape(-1, S, G).contiguous()
        X_pert_hat = X_pert_hat[:, :, self.hvg_mask]
        pert_cell_g = pert_cell_g[:, :, self.hvg_mask]

        X_pert_emb_hat = X_pert_emb_hat.reshape(-1, S, D).contiguous()
        pert_cell_emb = pert_cell_emb.reshape(-1, S, D).contiguous()

        # MMD loss
        dist_loss = self.mmd_loss(X_pert_hat, pert_cell_g).nanmean()
        dist_emb_loss = self.mmd_loss(X_pert_emb_hat, pert_cell_emb).nanmean()

        loss = 0.1 * dist_loss + dist_emb_loss
        # Log metrics
        self.log("train_loss", loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=B)
        self.log("train_dist_loss", dist_loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=B)
        self.log("train_dist_emb_loss", dist_emb_loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=B)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_dist_loss", dist_loss, on_step=False, on_epoch=True, prog_bar=True, batch_size=B)
        self.log("val_dist_emb_loss", dist_emb_loss, on_step=False, on_epoch=True, prog_bar=True, batch_size=B)
